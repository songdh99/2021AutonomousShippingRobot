...
class Transform():
    def __init__(self):
        self.a_about_r_pub = rospy.Publisher('a_about_m_pos', Pose, queue_size=10)
        ...
        self.br = tf.TransformBroadcaster(queue_size=10)
    ...
    def tf_pub(self):
        if self.aruco_check:
            rospy.loginfo_once("aruco tf start")
            self.draw_rgb()
            (trans, rot) = self.listener.lookupTransform('/base_footprint', 'aruco_pos', rospy.Time(0))
            ...
            rospy.loginfo_once("trans {}  rot {}".format(trans, rot))
            self.a_about_r_pub.publish(self.pos)
            rospy.loginfo_once("aruco tf end")

    #mani_pos에 따른 캠에서 로봇으로 tf
    def draw_cam(self, mani_pose):
        rospy.loginfo_once("draw_cam")
        rospy.loginfo_once("{}".format(mani_pose))
        ...
        self.br.sendTransform((t[0]+), q, rospy.Time.now(), "cam_test", "/base_footprint")
    ...
    def draw_aruco(self, aruco_pos):
        ...
        rospy.loginfo_once("{}".format(aruco_pos))
